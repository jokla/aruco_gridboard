...
void Node::spin(){
  ...
  while(ros::ok()){
    ...
    if (lastHeaderSeq_ != image_header_.seq)
    {
      ...
      // std::cout <<"Picture num" <<  image_header_.seq << std::endl;
      cv::Mat imageCopy;
      ...
      // bool status_tracker =  0;

      //            std::vector< int > markerIds;
      //            std::vector< std::vector<cv::Point2f> > markerCorners;
      //            cv::aruco::detectMarkers(imageCopy, board_->dictionary, markerCorners, markerIds);

      //            //std::cout << "Found  " << markerIds.size() << " markers." << std::endl;

      //            // if at least one marker detected
      //            cv::Vec3d rvec, tvec;
      //            if(markerIds.size() > 0) {
      //                status_tracker = cv::aruco::estimatePoseBoard(markerCorners, markerIds, board_, camMatrix_, distCoeffs_, rvec, tvec);
      //            }

      //            //std::cout<< "Cam" << std::endl << camMatrix_ << std::endl;
      ...
      ////                cv::aruco::estimatePoseSingleMarkers(markerCorners, 0.17, camMatrix_, distCoeffs_, rvecs, tvecs);

      ////            for(unsigned int i = 0; i < markerIds.size(); i++)
      ////                cv::aruco::drawAxis(imageCopy, camMatrix_, distCoeffs_, rvecs1[i], tvecs1[i], 0.17 * 0.5f);
      ...
      //                cv::aruco::drawAxis(imageCopy, camMatrix_, distCoeffs_, rvec, tvec,  0.1);
      //            }
      ...
      std::cout << "Detection pose"<< std::endl;
      cv::Vec3d rvec, tvec;
      ...
      // std::cout << "dictionary:" << board_->dictionary << std::endl;
      ...
      cv::projectPoints(objPoints_file[1], rvec, tvec, camMatrix_, distCoeffs_, imagePoints);
      //for (unsigned int i = 0; i < imagePoints.size(); ++i)
      //  {
      cv::circle(imageCopy,imagePoints[0],4,cv::Scalar(100, 0, 255),2,8,0);
      cv::circle(imageCopy,imagePoints[1],4,cv::Scalar(255, 0, 0),2,8,0);
